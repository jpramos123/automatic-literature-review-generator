...
class Graph:
  ...
  def create_graph(self, sentences, words_dict):
    temp_graph = {}
    for sentence in sentences:
      ...
      for word_idx in range(len(sentence)-1):
        ...
        if current_word not in temp_graph: # Make sure all words are a key in the dictionary
    ...
    for node in temp_graph:
      ...
      for connected_node in connected_nodes:
        if (node, connected_node) not in visited_nodes:
          if temp_graph[connected_node]: # Se o no conectado ao no principal nao eh vazio
            if node in temp_graph[connected_node]:
              edge_value = temp_graph[node][connected_node] + temp_graph[connected_node][node]
              #print(edge_value)
            ...
            temp_graph[connected_node][node] = temp_graph[node][connected_node]
  ...
  def compute_edges_prob(self):
    ...
    for out_node in self.graph: # reference node
      out_node_weight = 0
      for in_node in self.graph[out_node]: # all other nodes inside the reference node
    ...
    for out_node in self.graph:
      for in_node in self.graph[out_node]:
        ...
        edge_prob = edge_weight / (nodes_weight[out_node] + nodes_weight[in_node] - edge_weight)
